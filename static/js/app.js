...
async function insertData() {
    ...
    try {
        ...
        if (result.success) {
            ...
            refreshSystemState();
            setTimeout(() => {
                showVersionChain(result.row_id);
            }, 100); // 短暂延迟以确保UI更新完成
        } else {
...
async function deleteData() {
    ...
    try {
        ...
        if (result.success) {
            ...
            readResultDiv.style.display = 'none';
...
async function readData() {
    ...
    try {
        ...
        if (result.success) {
            // 在按钮旁边显示读取结果
            const readResultDiv = document.getElementById('readResult');
            const readResultContent = document.getElementById('readResultContent');
            
            if (result.data) {
                readResultContent.innerHTML = `<div class="read-result-content">${JSON.stringify(result.data)}</div>`;
                showMessage(`读取成功: ${JSON.stringify(result.data)}`, 'success');
            } else {
                readResultContent.innerHTML = `<div class="read-result-content">该行数据对当前事务不可见</div>`;
                showMessage('该行数据对当前事务不可见', 'info');
            }
            
            readResultDiv.style.display = 'block';
            
            // 检查是否需要显示读取路径弹窗
            const showReadPathCheckbox = document.getElementById('showReadPath');
            if (showReadPathCheckbox.checked && result.path) {
                // 显示读取路径（弹窗形式）
...
async function refreshSystemState() {
    try {
        ...
        if (selectedRowId !== null) {
            // 检查选中的行是否还存在
            const rowExists = state.rows.some(row => row.row_id === selectedRowId);
        ...
        // 处理版本链显示
        if (shouldClearVersionChain) {
            // 清空版本链显示
            const container = document.getElementById('versionChain');
            container.innerHTML = '<p style="color: #718096; text-align: center;">该数据行已被删除</p>';
            selectedRowId = null; // 清除选中状态
        } else if (shouldRefreshVersionChain && selectedRowId !== null) {
            // 只在选中行被修改时才刷新版本链
                ...
                <tbody>
                    ${rows.map(row => `
                        <tr class="data-table-row ${row.deleted ? 'deleted-row' : ''}"
                            ...
                            <td class="innodb-value ${row.display_roll_pointer ? '' : 'null-value'}">${row.display_roll_pointer || 'NULL'}</td>
                            ${dataColumns.map(col => `
...
function renderUndoLogs(undoLogs) {
    ...
    // 按row_id分组Undo日志，构建版本链
    const undoByRow = {};
    undoLogs.forEach(log => {
        if (!undoByRow[log.row_id]) {
            undoByRow[log.row_id] = [];
        }
        undoByRow[log.row_id].push(log);
    });

    // 为每个row_id构建版本链
    let html = '';
    Object.keys(undoByRow).forEach(rowId => {
        const rowUndos = undoByRow[rowId];

        // 按undo_id降序排列（最新的在前）
        rowUndos.sort((a, b) => b.undo_id - a.undo_id);

        html += `
            <div class="undo-chain-container">
                <div class="undo-chain-header">
                    <h4>行 #${rowId} 的版本链</h4>
                    <span class="undo-chain-count">${rowUndos.length} 个版本</span>
                </div>
                <div class="undo-chain-list">
        `;

        rowUndos.forEach((log, index) => {
            const hasNext = index < rowUndos.length - 1;

            html += `
                <div class="undo-log-with-arrow">
                    <div class="undo-log">
                        <!-- Undo Log Header: 关键MVCC字段 -->
...
async function showVersionChain(rowId) {
    try {
        ...
        if (response.status === 404) {
            const container = document.getElementById('versionChain');
            container.innerHTML = '<p style="color: #718096; text-align: center;">该数据行已被删除或不存在</p>';
            selectedRowId = null; // 清除选中状态
            return;
        }
        ...
        container.innerHTML = `
            ...
            <div class="version-chain-container">
                ${reversedVersions.map((version, index) => {
                    ...
                    // 提取用户数据列
                    const dataColumns = Object.keys(version.data);
                                ...
                                <table class="data-table version-table">
                                    ...
                                    <tbody>
                                        <tr class="data-table-row">
                                            <td class="innodb-value">${rowId}</td>
                                            <td class="innodb-value">${version.trx_id}</td>
                                            <td class="innodb-value ${undoLog && undoLog.roll_pointer ? '' : 'null-value'}">${undoLog && undoLog.roll_pointer ? undoLog.roll_pointer : 'NULL'}</td>
                                            ${dataColumns.map(col => `
                                                <td class="user-value">${version.data[col] !== undefined ? JSON.stringify(version.data[col]) : 'NULL'}</td>
                                            `).join('')}
                                        </tr>
                                    </tbody>
                                </table>
            ...
            <div class="split-data-row ${visible ? '' : 'invisible'}">
                ...
                <div class="innodb-hidden-fields">
                    ...
                    <div class="hidden-field">
                        ...
                        <span class="hidden-field-value">${row.display_roll_pointer || 'NULL'}</span>
                    </div>
                </div>
